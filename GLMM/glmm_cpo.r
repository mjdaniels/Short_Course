...
for (i in 1:N1) {
  ...
  data=list(y=y.3,x1=x.11,x2=x.22,trt=trt.3,id=id.3,N=nrec.3,N1=nsub.3,
            ynew=ynew,xnew1=xnew1,xnew2=xnew2,nobs=nobs,trtnew=trtnew)
  
  para<-c("ynewlogli")
  
  glmmcpo<-jags(data=data, inits=NULL, parameters.to.save=para, model.file="D:\\dandan\\short course\\examples\\CTQ\\glmm_cpo.txt",
